      ...
      <div 
        ...
        <div
          ...
          <ArticleCardSimple
            ...
            :key="article.Id"
            :article="article"
            :OLDurl="`/lag/${$route.params.league}/${$route.params.team}/${article.UrlSafeName.replace(/ø/gi,'o')}`"
  ...
  async fetch () {
    ...
    try {
      const [a, p, c, s, g, b] = await Promise.all([
        this.$axios.$get(
          '/webapi/'+this.$i18n.locale+'/Article/getArticleList?sortorder=0&pageSize=0&lookUpBrand=false&brand='+brand+'&attribute=null&teamList=null&color='+color+'&size='+size+'&gender='+gender+'&productType='+productType+'&sale='+sale+'&pageNum='+ pageNum +'&seoName=' +this.$route.params.team
        ),
        this.$axios.$get(
          '/webapi/'+this.$i18n.locale+'/Filter/GetProductTypeList?seoName='+this.$route.params.league+'&teamName='+this.$route.params.team+'&brandName=null'
        ),
        this.$axios.$get(
