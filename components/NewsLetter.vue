...
export default {
  ...
  async fetch () {
    try{
      let [storyblok] = await Promise.all([
          this.$axios.$get("https://api.storyblok.com/v1/cdn/datasource_entries?dimension="+ this.$i18n.locale +"&datasource=fe-labels-newsletter&token="+process.env.STORYBLOK +"&cv="+this.$store.getters.version)
      ]);
  ...
  methods:{
    async newsletter(event) {
        ...
        var _this = this
        await this.$axios.post(process.env.API_URL + '/webapi/'+this.$i18n.locale+'/Signup/PostNewsLetterSignup?email='+this.email)
