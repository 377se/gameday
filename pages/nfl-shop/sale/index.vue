      ...
      <div class="uk-container uk-container-large uk-padding-small">
        ...
        <div
          ...
          <ArticleCardSimple
            ...
            :key="article.Id"
            :article="article"
            :OLDurl="`/lag/nfl/${article.TeamNameSeo}/${article.UrlSafeName.replace(/ø/gi,'o')}`"
  ...
  async fetch () {
    ...
    try {
      const [a, p, c, s, g, b, sb] = await Promise.all([
        this.$axios.$get(
          '/webapi/'+this.$i18n.locale+'/Article/getArticleList?sortorder=0&pageSize=0&lookUpBrand=false&brand='+brand+'&attribute=null&teamList='+team+'&color='+color+'&size='+size+'&gender='+gender+'&productType='+productType+'&sale=true&pageNum='+ pageNum +'&seoName=nfl'
        ),
        this.$axios.$get(
