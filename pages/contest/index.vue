...
export default {
  async fetch () {
    ...
    try{
      let [contest] = await Promise.all([
        this.$axios.$get(process.env.API_URL + '/webapi/'+this.$i18n.locale+'/contest')
      ]);
  ...
  methods:{
    async postContest() {
      ...
      this.errors = []
      var _this = this
      await this.$axios.post(process.env.API_URL + '/webapi/'+this.$i18n.locale+'/Contest', _this.contestDTO)
