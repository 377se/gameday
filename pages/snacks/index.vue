    ...
    <template v-else>
      <div class="uk-container uk-container-large uk-padding-small">
        <ul class="uk-breadcrumb">
          ...
          <li><nuxt-link :to="localePath('/mlb-shop')">Snacks-shop</nuxt-link></li>
        </ul>
      </div>
      <component 
  ...
  // Check if we are in the editor mode
    ...
    try {
      const [sb] = await Promise.all([
        this.$storyapi.get('cdn/stories?starts_with=' + process.env.STORYBLOK_CATALOGUE.replace('/','') + '/' +this.$i18n.locale+ '/snacks', {
          version: version,
