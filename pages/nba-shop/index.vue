    ...
    <template v-else>
      <div class="uk-container uk-container-large uk-padding-small">
        <ul class="uk-breadcrumb">
          ...
          <li><a :href="localePath('/nba-shop')" @click.stop.prevent="$router.push({path:localePath('/nba-shop')})">NBA-shop</a></li>
        </ul>
      </div>
      <component 
  ...
  // Check if we are in the editor mode
    ...
    try {
      const [sb] = await Promise.all([
        this.$storyapi.get('cdn/stories?starts_with=' + process.env.STORYBLOK_CATALOGUE.replace('/','') + '/' +this.$i18n.locale+ '/nba-shop', {
          version: version,
