  ...
  async fetch () {
    ...
    try {
      const [sb] = await Promise.all([
        this.$storyapi.get('cdn/stories?is_startpage=1&level=1&sort_by=slug:desc&starts_with=' + process.env.STORYBLOK_CATALOGUE.replace('/','') + '/' +this.$i18n.locale+ '/c/'+this.$route.params.parentid+'/'+this.$route.params.categoryid, {
          version: version,
